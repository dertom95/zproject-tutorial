...
{
    test_item_t *item;
    printf ("Running gs selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef GS_BUILD_DRAFT_API // selftest is still in draft
        else
            gs_private_selftest (verbose, item->subtest);
#endif // GS_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    if (test) {
        printf ("Running gs test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef GS_BUILD_DRAFT_API // selftest is still in draft
        else
            gs_private_selftest (verbose, test->subtest);
#endif // GS_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
